...
class SpreadsheetApp:
    ...
    def formula_eval(self,row=-1,col=-1,deselect=True):
        ...
        if (row,col) and value:
            ...
            if(value[0] == '='):
                try:
                  ...
                  #  print(value)
                    var,self.graph=(evaluate(value[1:],(row,col),self.keys,self.graph))
                    self.variables[(row,col)].set(var)
                    print(row,col)
                    print(self.graph)
                    try:
                        for cell in self.graph[(row,col)]:
                            red,kol=cell
                            print(cell)
                            self.formula_eval(red,kol,deselect=False)
                        #self.focus_cell(row,col,update=False)
                    except KeyError:
                        pass
                   ...
                   # print(self.keys)
                    ...
                    self.variables[(row,col)].set("#REF!")
                except:
    ...
    def focus_cell(self, row, col, update=True):
        
            
        self.entries[(row, col)].focus_set()
        
        self.deselect_all_cells(update)
    ...
    def deselect_all_cells(self,update=True):
        if self.selected_cell:
